...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarVec = x1 * x2 + y1 * y2;
  const modulesVector1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modulesVector2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  if (modulesVector1 === 0 || modulesVector2 === 0) {
    return 0;
  }

  const cosTheta = scalarVec / (modulesVector1 * modulesVector2);
